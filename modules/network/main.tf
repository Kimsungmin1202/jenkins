...
resource "aws_subnet" "public" {
  ...
  tags = merge(var.tags, { Name = "${var.vpc_name}-public-${each.key}" , "kubernetes.io/role/elb" = "1" })
}
...
resource "aws_subnet" "private" {
  ...
  cidr_block        = each.value.cidr
  availability_zone = each.value.az
  tags = merge(var.tags, { Name = "${var.vpc_name}-private-${each.key}", "kubernetes.io/role/internal-elb" = "1" })
}
